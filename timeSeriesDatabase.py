...
class TimeSeriesDatabase:
    ...
    def get_samples_test_np(self, data_pack, labels, shuffle=False):
        dataset = SlidingWindowDataset(data_pack, labels, self.args.lookback, self.args, self.args.target_dims)
...
class SlidingWindowDataset(Dataset):
    def __init__(self, data, label, window, args, target_dim=None, horizon=1):
        self.data = data
        self.label = label
        self.window = window
